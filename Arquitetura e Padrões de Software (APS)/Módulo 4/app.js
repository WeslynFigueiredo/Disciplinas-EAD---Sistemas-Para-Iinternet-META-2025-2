...
// Activity scores
let activityScores = {
  flashback: null,    // Flashback quiz
  activity1: 0,       // Structural patterns scenarios
  activity2: 0,       // Behavioral patterns scenarios
  activity3: null,    // Final activity problem 1
  activity4: null     // Final activity problem 2
};
...
tocCards.forEach((card, index) => {
    card.addEventListener('click', () => {
        ...
        const startingSlides = [3, 4, 13, 23];
...
// Activity Flashback: Flashback quiz
function initFlashbackQuiz() {
  const options = document.querySelectorAll('[data-activity="flashback"]');
  ...
  options.forEach(option => {
    option.addEventListener('click', function() {
      ...
      const isCorrect = this.dataset.correct === 'true';
      const feedbackBox = document.getElementById('flashback-feedback');
      ...
      if (isCorrect) {
        this.classList.add('correct');
        activityScores.flashback = 1;
        feedbackBox.className = 'feedback-box success';
        feedbackBox.innerHTML = '✅ CORRETO! No Módulo 3 estudamos os Padrões CRIACIONAIS (Factory Method e Singleton), que lidam com a criação de objetos!';
      } else {
        this.classList.add('incorrect');
        activityScores.flashback = 0;
        feedbackBox.className = 'feedback-box error';
        feedbackBox.innerHTML = '❌ Não foi essa! No Módulo 3 vimos CRIACIONAIS. Agora vamos estudar Estruturais e Comportamentais!';
      }
...
// Activity 1: Adapter or Facade scenarios (slide 12)
function initActivity1() {
  const classifierButtons = document.querySelectorAll('.classifier-btn');
  
  const explanations = {
    1: {
      correct: '✅ ADAPTER! Você precisa adaptar uma interface incompatível (biblioteca nova) para funcionar com seu código existente.',
      incorrect: '❌ Facade simplifica acesso a subsistemas complexos. Aqui o problema é INCOMPATIBILIDADE de interfaces - use Adapter!'
    },
    2: {
      correct: '✅ FACADE! Você quer SIMPLIFICAR o acesso a múltiplos subsistemas complexos com uma interface única e fácil.',
      incorrect: '❌ Adapter adapta interfaces incompatíveis. Aqui você quer SIMPLIFICAR complexidade - use Facade!'
    }
  };
  ...
  classifierButtons.forEach(button => {
    button.addEventListener('click', function() {
      ...
      if (scenario === '1' || scenario === '2') {
        if (isCorrect) {
          activityScores.activity1 = (activityScores.activity1 || 0) + 1;
        }
      }
    });
  });
...
// Activity 2: Observer or Strategy scenarios (slide 22)
function initActivity2() {
  const classifierButtons = document.querySelectorAll('.classifier-btn');
  
  const explanations = {
    3: {
      correct: '✅ OBSERVER! Quando o status muda (evento), MÚLTIPLOS sistemas precisam ser notificados automaticamente (e-mail, dashboard, app, log).',
      incorrect: '❌ Strategy troca algoritmos. Aqui é sobre NOTIFICAR múltiplos sistemas sobre um evento - use Observer!'
    },
    4: {
      correct: '✅ STRATEGY! Cliente escolhe UMA opção de frete, e o sistema usa o ALGORITMO correspondente para calcular o preço.',
      incorrect: '❌ Observer notifica múltiplos objetos. Aqui você quer TROCAR algoritmo de cálculo baseado na escolha - use Strategy!'
    }
  };
  ...
  classifierButtons.forEach(button => {
    button.addEventListener('click', function() {
      ...
      if (scenario === '3' || scenario === '4') {
        if (isCorrect) {
          activityScores.activity2 = (activityScores.activity2 || 0) + 1;
        }
      }
    });
  });
...
// Final Activity Problem 1 (slide 25)
function initFinalActivity1() {
  const options = document.querySelectorAll('[data-activity="final1"]');
  ...
  options.forEach(option => {
    option.addEventListener('click', function() {
      ...
      const isCorrect = this.dataset.correct === 'true';
      const feedbackBox = document.getElementById('final1-feedback');
      ...
      if (isCorrect) {
        ...
        activityScores.activity3 = 1;
        feedbackBox.className = 'feedback-box success';
        feedbackBox.innerHTML = '🎯 CORRETO! FACADE é perfeito para simplificar acesso a múltiplos subsistemas complexos. O médico terá uma interface simples que orquestra todos os módulos internamente. Veja a resolução detalhada no próximo slide!';
      } else {
        this.classList.add('incorrect');
        activityScores.activity3 = 0;
        feedbackBox.className = 'feedback-box error';
        feedbackBox.innerHTML = '❌ Não é essa! Pense: o problema é COMPLEXIDADE de múltiplos subsistemas. Você quer SIMPLIFICAR com uma interface unificada. Qual padrão faz isso? Veja a resolução no próximo slide.';
      }
...
// Final Activity Problem 2 (slide 27)
function initFinalActivity2() {
  const options = document.querySelectorAll('[data-activity="final2"]');
  if (options.length === 0) return;
  ...
  options.forEach(option => {
    option.addEventListener('click', function() {
      ...
      const isCorrect = this.dataset.correct === 'true';
      const feedbackBox = document.getElementById('final2-feedback');
      ...
      if (isCorrect) {
        ...
        activityScores.activity4 = 1;
        feedbackBox.className = 'feedback-box success';
        feedbackBox.innerHTML = '🎯 PERFEITO! OBSERVER é ideal para notificar múltiplos módulos independentes sobre um evento. Quando o vídeo inicia, todos os observers são notificados automaticamente. Veja a resolução detalhada no próximo slide!';
      } else {
        this.classList.add('incorrect');
        activityScores.activity4 = 0;
        feedbackBox.className = 'feedback-box error';
        feedbackBox.innerHTML = '❌ Não é essa! Pense: 1 evento (play) precisa NOTIFICAR MÚLTIPLOS módulos independentes. Qual padrão faz comunicação 1-para-muitos? Veja a resolução no próximo slide.';
      }
